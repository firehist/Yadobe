 ...
 * @since 31/08/2011
 * @param RGB
 *            String RGB Value : x, x, x (x entre 0 et 255)
 * @param HEX
 *            String Hex Value : xxxxxx (x entre 0 et F)
 * @param HSV
...
var Tools = {
	...
	randomFromArray : function(arr) {
		if (!arr instanceof Array) {
			throw 'Parameter of randomFromArray() must be an array';
		}
		var randVal = Tools.randomXToY(0, arr.length - 1);
		return arr[randVal];
	},
	increaseColor : function(color, value) {
		...
		return '#' + red.toString(16).lpad(2, '0')
				+ green.toString(16).lpad(2, '0')
				+ blue.toString(16).lpad(2, '0');
	},
	...
	arrayKeys : function(object) {
		...
		if (typeof object == "object") {
			for (var propertyName in object) {
				if ((object instanceof Array) || (object.hasOwnProperty(propertyName))) {
	...
	arrayValues : function(object) {
		var accumulator = [];
		if (typeof object == "object") {
			for (var propertyName in object) {
				if ((object instanceof Array) || (object.hasOwnProperty(propertyName))) {
					accumulator.push(object[propertyName]);
				}
			}
		}
		return accumulator;
